...
const MESSAGES = Object.freeze({
  ...
  [SHAPE.ALLOWED_KEYS]: ({ label, value, args }) => 
    `${label} contains: ${fp.keys(value).join(', ')}, but should contain only keys: ${args.join(', ')}`,
  /**
   * @memberof module:ego~ERROR_MESSAGES
   * 
   ...
   * // Superfield contains [] should have keys: prop1, prop2
   * 
   */
  [SHAPE.EXPECTED_KEYS]: ({ label, value, args }) => 
    `${label} contains: ${fp.keys(value).join(', ')}, but should have keys: ${args.join(', ')}`,
  /**
   * @memberof module:ego~ERROR_MESSAGES
   * 
